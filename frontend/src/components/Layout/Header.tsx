    ...
    <header className="shrink-0 h-14 sm:h-16 bg-white/95 dark:bg-slate-950/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-3 sm:px-6 flex items-center justify-between gap-2 sm:gap-3 sticky top-0 z-20 transition-colors duration-200">
      ...
      <div className="flex items-center gap-2 min-w-0 shrink">
        ...
        <button
          ...
          className="md:hidden p-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition shrink-0 active:scale-95"
          title="Open Vault Navigation"
        >
          <Menu className="w-5 h-5" />
        </button>

        <div className="md:hidden flex items-center gap-1.5 font-black text-xs sm:text-sm tracking-tight text-slate-900 dark:text-white">
          <span className="bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Looser</span>
          <span className="text-[9px] px-1.5 py-0.2 rounded-md bg-blue-500/10 text-blue-600 dark:text-blue-400 font-mono font-medium border border-blue-500/20">Vault</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 px-1 text-xs font-medium text-slate-700 dark:text-slate-300">
          <Clock className="w-3.5 h-3.5 text-blue-500 shrink-0" />
          <span className="font-mono text-xs font-semibold text-slate-900 dark:text-white">
            {currentTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
          </span>
          <span className="hidden sm:inline text-slate-300 dark:text-slate-600">|</span>
          <span className="hidden sm:inline font-medium text-slate-600 dark:text-slate-300 text-[11px]">
