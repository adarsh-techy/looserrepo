...
export async function getAllUsers(req: AuthenticatedRequest, res: Response) {
  ...
  const parsedUsers = users.map((u) => {
    let perms: string[] = ['*'];
    try {
      perms = JSON.parse(u.pagePermissions || '["*"]');
    } catch (e) {
      perms = ['*'];
    }
...
export async function createUser(req: AuthenticatedRequest, res: Response) {
  ...
  try {
    parsedPerms = JSON.parse(newUser.pagePermissions || '["*"]');
  } catch (e) {
    parsedPerms = ['*'];
  }
...
export async function updateUserPermissions(req: AuthenticatedRequest, res: Response) {
  ...
  let parsedPerms: string[] = [];
  try {
    parsedPerms = JSON.parse(updatedUser.pagePermissions || '[]');
  } catch (e) {
    parsedPerms = [];
  }
...
export async function getPartnerStatus(req: AuthenticatedRequest, res: Response) {
  ...
  const diffMs = Date.now() - mostRecentTimestamp;
  const hoursSinceCheckIn = Math.max(0, Math.floor(diffMs / (1000 * 60 * 60)));
  const minutesSinceCheckIn = Math.max(0, Math.floor((diffMs % (1000 * 60 * 60)) / (1000 * 60)));
